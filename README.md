...
**汽车仪表盘样式**的表盘：240° 刻度盘（刻度线 + 刻度数字）、可选彩色分区、指针与
数字读数（含单位）。HA 自带 `gauge` 卡片的轻量替代，另针对本设备做了两点补齐：
...
min: 0                    # 可选，默认 0
decimals: 2               # 可选，默认 2
ticks: 5                  # 可选，刻度数字个数（默认 5）
severity:                 # 可选，彩色分区（显示单位）
  red: 0
  yellow: 2
  green: 5
```

- 刻度数字的小数位按**实际步长**推导，并去掉多余的尾零：`max: 10` + `ticks: 5`
  得到 `0 / 2.5 / 5 / 7.5 / 10`；`ticks: 3` 得到 `0 / 5 / 10`。
- `severity` 画成刻度盘内侧的彩色分区（不填则只有进度弧）。
